...
from utils import SpecialEncoder, SpecialEncoderConfig
...
class ModelWrapper(torch.nn.Module):
    def __init__(self, unet, special_encoder) -> None:
        super().__init__()
        self.unet=unet
        self.special_encoder=special_encoder
        
    def forward(self, input, task):
        if task=="encoding":
            ret=self.special_encoder(*input)
        elif task=='denoising':
            ret = self.unet(*input).sample
        return ret
    
    def requires_grad_(self, requires_grad):
        self.unet.requires_grad_(requires_grad)
        self.special_encoder.requires_grad_(requires_grad)
        return self
    
    def train(self, mode=True):
        self.unet.train(mode)
        self.special_encoder.train(mode)
        return self
...
def main():
    ...
    print(f"using vae from <<{vae_path}>>")
    vae = AutoencoderKL.from_pretrained(
        vae_path, revision=args.revision
    )
    ...
    special_encoder=SpecialEncoder(**SpecialEncoderConfig)
    
    if args.load_pretrained_weights!=None :
        unet=UNet2DConditionModel.from_pretrained(args.load_pretrained_weights, subfolder='unet',)
    else :
    ...
    model = ModelWrapper(unet, special_encoder)
    ...
    if args.use_ema:
        ema_model = EMAModel(get_trainable_parameters(model), args.ema_decay)
    ...
    optimizer = optimizer_cls(
        ...
        [
            {"params":get_trainable_parameters(model.unet), "lr":args.learning_rate},
            {"params":get_trainable_parameters(model.special_encoder), "lr":args.spenc_learning_rate},
        ],
    ...
    # TODO : 检查 dataset 加载的情况
    with accelerator.main_process_first():
        train_dataset = Dataset(
            "train", tokenizer, [args.resolution], args
        )
        

    # DataLoaders creation:
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset, shuffle=True, batch_size=args.train_batch_size, num_workers=args.dataloader_num_workers
    )
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(model):
                with accelerator.autocast():
                    ...
                    real=batch[args.resolution]['real']
                    mask=batch[args.resolution]['mask']
                    comp=batch[args.resolution]['comp']
                    caption_ids=batch[args.resolution]['caption_ids']
                    
                    if args.resolution != args.infer_resolution:
                        # * 默认：加载出来的图片分辨率小，缩放到大分辨率进行训练
                        tgt_size=[args.infer_resolution, args.infer_resolution]
                        real = TF.resize(real, size=tgt_size, interpolation=InterpolationMode.BICUBIC, antialias=True)
                        comp = TF.resize(comp, size=tgt_size, interpolation=InterpolationMode.BICUBIC, antialias=True)
                        mask = TF.resize(mask, size=tgt_size, interpolation=InterpolationMode.NEAREST, antialias=True)
                        mask = (mask > 0.5).float()
                    ...
                    masked_image_latents = model((comp, mask), task='encoding')
                    # masked_image_latents = vae.encode(comp.to(weight_dtype)).latent_dist.sample()
                    # masked_image_latents = masked_image_latents * vae.config.scaling_factor
                    
                    vae_scale_factor = 2 ** (len(vae.config.block_out_channels) - 1)
                    mask = torch.nn.functional.interpolate(
                        mask, size=(comp.shape[2] // vae_scale_factor, comp.shape[3] // vae_scale_factor)
                    )
                    ...
                    model_pred = model((latent_model_input, timesteps, encoder_hidden_states), task='denoising')
                    # model_pred = model(latent_model_input, timesteps, encoder_hidden_states).sample
                    ...
                    if args.nofg:
                        ...
                        loss = F.mse_loss(model_pred.float()*mask, target.float()*mask, reduction="none").mean(dim=(2,3),keepdim=True)
                        loss_scale = ((mask.shape[2]*mask.shape[3]) / mask.sum(dim=(2,3), keepdim=True)).clamp(1,5)
                ...
                if accelerator.sync_gradients:
                    accelerator.clip_grad_norm_(get_trainable_parameters(model), args.max_grad_norm)
                optimizer.step()
                lr_scheduler.step()
                optimizer.zero_grad()
            ...
            if accelerator.sync_gradients:
                ...
                if (global_step % args.checkpointing_steps == 0) or (global_step == args.max_train_steps):
                    ...
                    if accelerator.is_main_process:
                        ...
                        unet_to_save = unwrapped_model.unet
                        ...
                        spenc_to_save = unwrapped_model.special_encoder
                        spenc_save_dir = os.path.join(
                            args.output_dir, f"weights-{global_step}", 'special_encoder')
                        os.makedirs(spenc_save_dir, exist_ok=True)
                        torch.save(spenc_to_save, os.path.join(spenc_save_dir, 'model.pt'))
